...
initMap();renderDomains();renderSimulation();saveQueue(queued());Promise.all([refreshEvents(),refreshTracks(),refreshDeconfliction(),refreshZones(),refreshAlerts(),refreshTasks(),refreshReadiness(),refreshComms(),refreshIntegrations()]);flushOffline();
setInterval(()=>Promise.all([refreshEvents(),refreshTracks(),refreshDeconfliction(),refreshTasks(),refreshReadiness(),refreshComms(),refreshIntegrations()]),5000);
</script></body></html>""")
