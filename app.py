...
<div class="panel"><h3>Track Directory</h3><div id="trackList"></div></div></div></section>
...
let map, mapMarkers=[], trackMarkers=[], simMarkers=[], simTrails=[];
...
async function flushOffline(){
 ...
 saveQueue(remaining);await Promise.all([refreshEvents(),refreshTracks(),refreshTasks(),refreshReadiness(),refreshComms()])
}
...
async function refreshTracks(){
 ...
 document.getElementById('trackList').innerHTML=tracks.length?tracks.map(t=>`<div class="row"><span class="track-id">${t.track_id}</span> <span class="pill track-${t.track_status}">${t.track_status.toUpperCase()}</span><br><b>${t.object_class}</b> · ${t.identification}<br><span class="muted">${t.domain} · ${t.source} · ${t.altitude_ft==null?'ALT —':'ALT '+Math.round(t.altitude_ft).toLocaleString()+' ft'} · ${t.speed_kt==null?'SPD —':'SPD '+Math.round(t.speed_kt)+' kt'}</span></div>`).join(''):'<div class="muted">No live tracks yet.</div>';
}
...
initMap();renderDomains();renderSimulation();saveQueue(queued());Promise.all([refreshEvents(),refreshTracks(),refreshTasks(),refreshReadiness(),refreshComms(),refreshIntegrations()]);flushOffline();
setInterval(()=>Promise.all([refreshEvents(),refreshTracks(),refreshTasks(),refreshReadiness(),refreshComms(),refreshIntegrations()]),5000);
</script></body></html>""")
