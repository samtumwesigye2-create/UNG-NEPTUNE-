...
async function flushOffline(){
 ...
 saveQueue(remaining);await Promise.all([refreshEvents(),refreshTracks(),refreshZones(),refreshTasks(),refreshReadiness(),refreshComms()])
}
...
initMap();renderDomains();renderSimulation();saveQueue(queued());Promise.all([refreshEvents(),refreshTracks(),refreshDeconfliction(),refreshZones(),refreshTasks(),refreshReadiness(),refreshComms(),refreshIntegrations()]);flushOffline();
